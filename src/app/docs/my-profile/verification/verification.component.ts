  ...
  <div class="modal-body">
    ...
    <form class="bootstrap-form needs-validation LoginForm" [formGroup]="verificationForm"
      ...
      <div class="form-group">
        ...
        <input type="text" maxlength="4" (keydown.space)="$event.preventDefault()" formControlName="changeDoctorOTP" class="form-control" id="verification"
          name="verification" placeholder="Enter verification code">
        <small class="text-danger"
          *ngIf="verificationForm.controls['changeDoctorOTP'].hasError('required') && verificationForm.controls['changeDoctorOTP'].dirty">This
          field is required</small>
        <small class="text-danger" *ngIf="verificationForm.controls['changeDoctorOTP'].hasError('pattern')">Accept
          only
          numeric
          value.</small>
      </div>
...
export class VerificationComponent implements OnInit {
  ...
  constructor(
    ...
    this.verificationForm = this.fb.group({
      changeDoctorOTP: [null, Validators.compose([Validators.required, Validators.pattern('^[0-9]*$')])]
    });
  }
  ...
  onSubmitOTP = (post: ChangeVerify) => {
    ...
    if (this.verificationForm.valid && invalidinput.length === 0) {
      const json = {
        languageID: '1',
        logindoctorID: this.list[0].res.logindoctorID,
        changeDoctorOTP: post.changeDoctorOTP.toString()
      };
