...
export class DocsPreviousComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.privious.startDate = this.service.getDocLocal() ? moment(this.service.getDocLocal().doctorCreatedDate).format('YYYY-MM-DD') : moment(this.service.getDocSession().doctorCreatedDate).format('YYYY-MM-DD');
  ...
  ngOnDestroy(): void {
    this.docService.destroyPrivious(); // destroying on-going subscription
